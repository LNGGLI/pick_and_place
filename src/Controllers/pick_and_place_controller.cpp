...
#include <pick_and_place/Controllers/pick_and_place_controller.h>
...
bool PickAndPlaceController::init(hardware_interface::RobotHW *robot_hardware,
  ...
  if (cartesian_pose_interface_ == nullptr) {
    ROS_ERROR("PickAndPlaceController: Could not get Cartesian Pose "
  ...
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR("PickAndPlaceController: Could not get parameter arm_id");
    return false;
  }
  ...
  try {
    ...
    ROS_ERROR_STREAM(
        "PickAndPlaceController: Exception getting Cartesian handle: "
        << e.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR(
        "PickAndPlaceController: Could not get state interface from hardware");
    return false;
  }

  command_pb_ =
      node_handle.advertise<trajectory_msgs::MultiDOFJointTrajectoryPoint>(
...
void PickAndPlaceController::update(const ros::Time & /* time */,
  ...
  if (!start || !traj_mutex.try_lock()) { // il controller è partito ma non è stata ancora assegnata
    ...
    sun::UnitQuaternion q =
        cartesian_traj_->getQuaternion(elapsed_time_.toSec());
...
PLUGINLIB_EXPORT_CLASS(PickAndPlaceController::PickAndPlaceController,
